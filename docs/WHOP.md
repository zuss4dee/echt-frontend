...
- After **`/auth/callback`**, users go to **`/analyze`** only when **`has_access`** and **`onboarding_complete`**. Otherwise they always go to **`/onboarding`**. The onboarding page checks access before redirecting to Analyze; if your profile is already complete but Whop has not synced yet, you stay on a short “Almost there” screen (confirming Whop, then Echt AI opens) instead of bouncing to the marketing subscribe flow (`/?subscribe=1…`).
