...
function LiquidEther({
  ...
  useEffect(() => {
    ...
    const paletteInput = [color0, color1, color2].filter(Boolean) as string[];
    const paletteTex = makePaletteTexture(paletteInput.length ? paletteInput : colors);
...
`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props: { cursor_size: number; mouse_force: number; cellScale: THREE.Vector2 }) {
        ...
        const cursorSizeX = props.cursor_size * cellScale.x;
        const cursorSizeY = props.cursor_size * cellScale.y;
